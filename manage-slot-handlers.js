...
const {
  ...
  listGroupsWithFreeSlots, nextOpenGroupLetters, groupTimeSummary,
...
async function handleManageSlotButton(interaction) {
  ...
  if (!options.length) {
    return interaction.reply({
      content: "❌ No other currently-open group has a free slot to switch into right now.",
      flags: MessageFlags.Ephemeral,
    });
  }
...
async function handleManageSlotGroupSelect(interaction) {
  ...
  await interaction.update({
    content: `✅ Moved **${already.team_name}** from **${groupDisplayName(oldGroup)}** to **${groupDisplayName(newGroup)}** — Slot ${localSlotNumber(newSlotNumber)}.`,
    components: [],
  });
}
